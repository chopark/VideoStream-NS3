 ...
 * 5. Wireless network(LTE) with 1 server and 3 mobile clients **FAILED**
 * 6. Wireless network with 1 servers and 10 mobile clients
 */
#define CASE 6
...
int main(int argc, char *argv[])
{
  ...
  else if (CASE == 3)
  {
    ...
    VideoStreamServerHelper videoServer(5000);
    videoServer.SetAttribute("MaxPacketSize", UintegerValue(1400));
    videoServer.SetAttribute("FrameFile", StringValue("./scratch/videoStreamer/random.txt"));
  ...
  else if (CASE == 5)
  {
    ...
    // LTE implementation
    // 
    const uint32_t nLte = 3, nAp = 1;
    Ptr<LteHelper> lteHelper = CreateObject<LteHelper> ();

    NodeContainer LteApNode;
    LteApNode.Create(nAp);
    NodeContainer LteStaNodes;
    LteStaNodes.Create(nLte);
    ...
    mobility.SetMobilityModel("ns3::RandomWalk2dMobilityModel", "Bounds", RectangleValue(Rectangle(-50, 50, -50, 50)));
    mobility.Install(LteApNode);

    mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
    mobility.Install(LteStaNodes);

    InternetStackHelper stack;
    stack.Install(LteApNode);
    ...
    NetDeviceContainer apDevs = lteHelper->InstallUeDevice (LteApNode);
    NetDeviceContainer staDevs = lteHelper->InstallEnbDevice (LteStaNodes);

    Ipv4InterfaceContainer apInterfaces;
    apInterfaces = address.Assign(apDevs);
    Ipv4InterfaceContainer lteInterfaces;
    lteInterfaces = address.Assign(staDevs);

    // lteHelper->Attach (staDevs, apDevs.Get (0));

    // enum EpsBearer::Qci q = EpsBearer::GBR_CONV_VOICE;
    // EpsBearer bearer (q);
    // lteHelper->ActivateDataRadioBearer (apDevs, bearer);
    ...
    for (uint m = 0; m < nAp; m++)
    {
      ApplicationContainer serverApps = videoServer.Install(LteApNode.Get(m));
      serverApps.Start(Seconds(0.0));
      serverApps.Stop(Seconds(100.0));
    }

    for (uint k = 0; k < nLte; k++)
    {
      VideoStreamClientHelper videoClient(lteInterfaces.GetAddress(k), 5000);
      ApplicationContainer clientApps =
          videoClient.Install(LteStaNodes.Get(k));
      clientApps.Start(Seconds(0.5));
      clientApps.Stop(Seconds(100.0));
    }
  }
